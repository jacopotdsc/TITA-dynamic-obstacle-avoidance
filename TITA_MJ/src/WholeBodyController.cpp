...
WholeBodyController::compute_inverse_dynamics(
  ...
  pinocchio::getFrameJacobianTimeVariation(robot_model_, robot_data_, base_link_idx_, pinocchio::ReferenceFrame::LOCAL_WORLD_ALIGNED, J_base_link_);
  pinocchio::getFrameJacobianTimeVariation(robot_model_, robot_data_, right_leg4_idx_, pinocchio::ReferenceFrame::LOCAL_WORLD_ALIGNED, J_right_wheel_);
  pinocchio::getFrameJacobianTimeVariation(robot_model_, robot_data_, left_leg4_idx_, pinocchio::ReferenceFrame::LOCAL_WORLD_ALIGNED, J_left_wheel_);
  ...
  const auto a_lwheel_drift = J_left_wheel_ * qdot;
  const auto a_rwheel_drift = J_right_wheel_ * qdot;
  const auto a_base_orientation_drift = J_base_link_.bottomRows<3>() * qdot;
  ...
  err_posture << Eigen::VectorXd::Zero(6), desired.qjnt - q.tail(q.size() - 7);
  Eigen::VectorXd err_posture_vel(6 + n_joints_); 
  err_posture_vel << Eigen::VectorXd::Zero(6), desired.qjntdot - qdot.tail(qdot.size() - 7);
  ...
  std::cout << "a_jnt_total " << a_jnt_total<< std::endl;
